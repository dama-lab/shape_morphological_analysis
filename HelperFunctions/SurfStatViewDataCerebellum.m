function [ a, cb ] = SurfStatViewDataCerebellum( vertexData, surfmesh, title, background, colormap)
...
t=size(surfmesh.tri,1);
v=size(surfmesh.coord,2);
tmax=max(surfmesh.tri,[],2);
tmin=min(surfmesh.tri,[],2);
...
clim=[min(vertexData),max(vertexData)];
if clim(1)==clim(2)
    clim=clim(1)+[-1 0];
end
...
r=max(surfmesh.coord,[],2)-min(surfmesh.coord,[],2);
w1=h/r(2)*r(1)*3/4;
h1=h/r(2)*r(1); % h/r(2)*r(3)

%%
a(1)=axes('position',[0.055 0.62 h*3/4 w]);
trisurf(surfmesh.tri(tl,:),surfmesh.coord(1,vl),surfmesh.coord(2,vl),surfmesh.coord(3,vl),...
    double(vertexData(vl)),'EdgeColor','none');
view(-90,0); 
%%
...
trisurf(surfmesh.tri,surfmesh.coord(1,:),surfmesh.coord(2,:),surfmesh.coord(3,:),...
    double(vertexData),'EdgeColor','none');
view(0,90); 
daspect([1 1 1]); axis tight; camlight; axis vis3d off;
...
if cut<t
    ...
    trisurf(surfmesh.tri(tr,:)-cuv,surfmesh.coord(1,vr),surfmesh.coord(2,vr),surfmesh.coord(3,vr),...
        double(vertexData(vr)),'EdgeColor','none');
    view(90,0);
    daspect([1 1 1]); axis tight; camlight; axis vis3d off;
    lighting phong; material shiny; shading interp;
else
